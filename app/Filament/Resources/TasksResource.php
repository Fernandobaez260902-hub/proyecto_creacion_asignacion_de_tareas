...
class TasksResource extends Resource
{
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('changeStatus')
                    ->label(function ($record): string {
                        ...
                        if ($user?->hasRole('desarrollador') && $record->status === 'in_progress') {
                    ...
                    ->action(function ($record) {
                        ...
                        if ($user?->hasRole('desarrollador') && $record->assigned_to === $user->id) {
                    ...
                    ->color(function ($record): string {
                        ...
                        return $record->status === 'pending' ? 'warning' : 'secondary';
                    })
                    ->visible(fn ($record): bool => static::canChangeTaskStatus($record)),
            ])
            ->filters([
                //
            ])
    ...
    protected static function canChangeTaskStatus(Tasks $record): bool
    {
        $user = static::currentUser();
        ...
        return $user?->hasRole('desarrollador')
            ...
            && in_array($record->status, ['pending', 'in_progress'], true);
    }
